...
_PRESETS = {
    ...
    "PushTPO-Full-Multimodal-v0": PushTPOConfig(
        obs_mode="full", shapes=["hexagon"], goal_margin=24.0
    ),
    "PushTPO-Lidar-Multimodal-v0": PushTPOConfig(shapes=["hexagon"], goal_margin=24.0),
    "PushTPO-Lidar-Multi3-v0": PushTPOConfig(
        n_objects=3, shapes=["square"], goal_margin=24.0
    ),
...
for _env_id, _config in _PRESETS.items():
    register(
        id=_env_id, entry_point="push_me.env:PushTPOEnv", kwargs={"config": _config}
    )
del _env_id, _config
