...
class ReplayBufferWriter:
    ...
    def n_episodes(self) -> int:
        return int(self._meta["episode_ends"].shape[0])

    @property
    def n_steps(self) -> int:
        ends = self._meta["episode_ends"]
    ...
    def append_episode(self, seed: int, steps: dict[str, np.ndarray]) -> None:
        ...
        ends = self._meta["episode_ends"]
        prev_end = int(ends[-1]) if ends.shape[0] else 0
        ends.resize((ends.shape[0] + 1,))
        ends[-1] = prev_end + n_new

        seeds = self._meta["episode_seeds"]
...
def read_config(path: str | Path) -> PushTPOConfig:
    ...
    return PushTPOConfig(**json.loads(root.attrs["config"]))
...
def read_episode(path: str | Path, index: int) -> dict[str, np.ndarray]:
    ...
    return {key: np.asarray(root["data"][key][start:end]) for key in root["data"].array_keys()}


def summarize_achieved_modes(path: str | Path) -> dict[int, int]:
    root = zarr.open_group(str(path), mode="r")
    ends = root["meta"]["episode_ends"][:]
    achieved_mode = np.asarray(root["data"]["achieved_mode"][:])
