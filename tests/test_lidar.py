...
from push_me.lidar import HitClass, N_HIT_CLASSES, cast_rays


def _static_segment(space: pymunk.Space, a, b, collision_type: HitClass, object_index: int | None = None) -> None:
...
def test_cast_rays_output_shapes():
    space = pymunk.Space()
    features, hit_object_index = cast_rays(space, origin=(0.0, 0.0), n_rays=16, lidar_range=100.0)
...
def test_ray_hits_wall_at_known_distance():
    ...
    features, hit_object_index = cast_rays(space, origin=(0.0, 0.0), n_rays=4, lidar_range=100.0)
...
def test_ray_index_corresponds_to_uniform_world_frame_angle():
    ...
    features, hit_object_index = cast_rays(space, origin=(0.0, 0.0), n_rays=8, lidar_range=100.0)
...
@given(st.integers(min_value=1, max_value=32), st.floats(min_value=1.0, max_value=500.0))
def test_empty_space_reports_max_range_and_none_for_every_ray(n_rays, lidar_range):
    space = pymunk.Space()
    features, hit_object_index = cast_rays(space, origin=(0.0, 0.0), n_rays=n_rays, lidar_range=lidar_range)
...
def test_shape_filter_excludes_masked_shapes():
    ...
    query_filter = pymunk.ShapeFilter(mask=pymunk.ShapeFilter.ALL_MASKS() ^ excluded_category)
