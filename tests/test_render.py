...
from push_me.render import BeliefMarker
...
def test_toggling_ground_truth_off_changes_the_frame():
    cfg = PushTPOConfig(n_objects=1, shapes=["square"], obs_mode="full", seed=0)
    env = PushTPOEnv(cfg, render_mode="rgb_array")
    env.reset(seed=0)
    frame_on = env.render()
    env._renderer.show_ground_truth = False
...
def test_snapshot_multi_object_recoloring_under_assignment(assert_matches_snapshot, sdl_dummy_driver):
    ...
    frame = env.render()
    assert_matches_snapshot("04_multi_object_assignment", frame)
    env.close()
...
def test_space_toggles_pause(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.paused is False
    _press(pygame.K_SPACE)
    env.render()
    assert env._renderer.paused is True
    _press(pygame.K_SPACE)
    env.render()
    assert env._renderer.paused is False
    env.close()
...
def test_l_toggles_lidar_rays(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.show_lidar is True
    _press(pygame.K_l)
    env.render()
    assert env._renderer.show_lidar is False
    env.close()
...
def test_b_toggles_belief_overlay(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.show_belief is True
    _press(pygame.K_b)
    env.render()
    assert env._renderer.show_belief is False
    env.close()
...
def test_g_toggles_ground_truth(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.show_ground_truth is True
    _press(pygame.K_g)
    env.render()
    assert env._renderer.show_ground_truth is False
    env.close()
...
def test_tab_sets_preset_cycle_request_for_play_py_to_consume(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.consume_preset_cycle_request() is False
    _press(pygame.K_TAB)
    env.render()
    assert env._renderer.consume_preset_cycle_request() is True
    assert env._renderer.consume_preset_cycle_request() is False
    env.close()
...
def test_unrecognised_keys_are_queued_for_external_consumers(sdl_dummy_driver):
    ...
    env = PushTPOEnv(cfg, render_mode="human")
    env.reset(seed=0)
    assert env._renderer.consume_unhandled_keys() == []
    _press(pygame.K_n)
    env.render()
    assert env._renderer.consume_unhandled_keys() == [pygame.K_n]
    assert env._renderer.consume_unhandled_keys() == []
    env.close()
